...
in
{
  ...
  config = {
    ...
    # Configure generic-extlinux-compatible for LoongArch64 SD card booting
    boot.loader.grub.enable = false;
    boot.loader.generic-extlinux-compatible.enable = true;

    boot.consoleLogLevel = lib.mkDefault 7;

    # LoongArch64 typically uses UEFI booting, but for SD cards we use extlinux
    boot.kernelParams = [
    ...
    # Set default populateRootCommands for generic-extlinux-compatible
    sdImage.populateRootCommands = ''
      mkdir -p ./files/boot
      ${config.boot.loader.generic-extlinux-compatible.populateCmd} -c ${config.system.build.toplevel} -d ./files/boot
    '';

    # Set default populateFirmwareCommands (empty for LoongArch64)
